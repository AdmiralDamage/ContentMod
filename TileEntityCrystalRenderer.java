package eu.tetrabyte.contentmod.renderer.tileentity;
...
public class TileEntityCrystalRenderer extends TileEntitySpecialRenderer<TileEntityCrystal>
{
    ...
    public void renderTileEntityAt(TileEntityCrystal te, double x, double y, double z, float partialTicks, int destroyStage)
    {
        ...
        if (f > 0.0F)
        {
            ...
            List<TileEntityCrystal.BeamSegment> list = te.getBeamSegments();
            int i = 0;
            ...
            for (int j = 0; j < list.size(); ++j)
            {
                ...
                double d3 = 0.2D;
                double d4 = 0.5D + Math.cos(d2 + 2.356194490192345D) * laserSize;
                double d5 = 0.5D + Math.sin(d2 + 2.356194490192345D) * laserSize;
                
                double d6 = 0.5D + Math.cos(d2 + (Math.PI / 4D)) * laserSize;
                double d7 = 0.5D + Math.sin(d2 + (Math.PI / 4D)) * laserSize;
                
                double d8 = 0.5D + Math.cos(d2 + 3.9269908169872414D) * laserSize;
                double d9 = 0.5D + Math.sin(d2 + 3.9269908169872414D) * laserSize;
                
                double d10 = 0.5D + Math.cos(d2 + 5.497787143782138D) * laserSize;
                double d11 = 0.5D + Math.sin(d2 + 5.497787143782138D) * laserSize;
                ...
                d11 = 1.0D;
                d12 = -1.0D + d1;
                d13 = (double)((float)TileEntityCrystal$beamsegment.getHeight() * f) + d12;
                worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
